...
CHECKPOINT ?=
OUTPUT_DIR ?= outputs
RAW_DIR ?= resources/data/shu-mi/mat_files

SHU_EDF_DIR ?= resources/data/shu-mi/edf_files
SHU_EVENTS_DIR ?= resources/data/shu-mi/events

SHU_DATASET ?= $(OUTPUT_DIR)/data/preprocessed/shu-mi/shu_mi.h5
HARMONIZED_SHU_DIR ?= $(OUTPUT_DIR)/data/harmonized/shu_mi
HARMONIZED_HBN_DIR ?= $(OUTPUT_DIR)/data/harmonized/hbn

HARMONIZED_SHU_MANIFEST ?= $(HARMONIZED_SHU_DIR)/manifest.parquet
HARMONIZED_SHU_EDF_DIR ?= $(HARMONIZED_SHU_DIR)/shu_mi_edf
...
DATALOADER_DATA ?= $(HARMONIZED_SHU_MANIFEST)
...
SHU_TARGET_RATE ?= 200
SKIP_INVALID ?= 0
SHU_EDF_SKIP_INVALID ?= 1
HBN_SKIP_INVALID ?= 1
HARMONIZE_WORKERS ?= 4
HARMONIZE_PROGRESS ?= 1
HARMONIZE_MAX_RECORDINGS_PER_JOB ?= 128
SHU_TARGET_JOB_GIB ?= 0.25
HBN_TARGET_JOB_GIB ?= 8
RESUME ?= 0
...
CBRAMOD_OUTPUT_DIR ?= $(OUTPUT_DIR)/cbramod_shu_mi
SIMPLECONV_OUTPUT_DIR ?= $(OUTPUT_DIR)/eegsimpleconv_shu_mi
TASK_C_OUTPUT_DIR ?= $(OUTPUT_DIR)/results_models_comparison
CBRAMOD_SUMMARY ?= $(CBRAMOD_OUTPUT_DIR)/summary.json
...
DATALOADER_BENCHMARK_OUTPUT ?= $(OUTPUT_DIR)/benchmarks/dataloader_epoch.json
DATALOADER_BATCH_SIZE ?= 64
DATALOADER_NUM_WORKERS ?= 8
DATALOADER_PREFETCH_FACTOR ?= 4
DATALOADER_SHUFFLE_BUFFER ?= 2048
DATALOADER_DEVICE ?= cuda
DATALOADER_EPOCH ?= 0
DATALOADER_PROGRESS ?= 1
DATA_NOTEBOOK ?= notebooks/shu_mi_data_exploration.ipynb
DATALOADER_NOTEBOOK ?= notebooks/harmonized_dataloader_benchmark.ipynb
PYTEST_ARGS ?=
OVERWRITE ?= 0
STRICT ?= 1
...
ifeq ($(SKIP_INVALID),1)
SKIP_INVALID_FLAG := --skip-invalid-recordings
else
SKIP_INVALID_FLAG :=
endif

ifeq ($(SHU_EDF_SKIP_INVALID),1)
SHU_EDF_INVALID_FLAG := --skip-invalid-recordings
else
SHU_EDF_INVALID_FLAG :=
endif

ifeq ($(HBN_SKIP_INVALID),1)
HBN_INVALID_FLAG := --skip-invalid-recordings
else
HBN_INVALID_FLAG :=
endif

ifeq ($(HARMONIZE_PROGRESS),0)
HARMONIZE_PROGRESS_FLAG := --no-progress
else
HARMONIZE_PROGRESS_FLAG :=
endif

ifeq ($(DATALOADER_PROGRESS),0)
DATALOADER_PROGRESS_FLAG := --no-progress
else
DATALOADER_PROGRESS_FLAG :=
endif

ifeq ($(RESUME),1)
HARMONIZE_RESUME_FLAG := --resume
else
HARMONIZE_RESUME_FLAG :=
endif

HARMONIZE_ENV := OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 NUMEXPR_NUM_THREADS=1
...
.PHONY: help all sync install lock update smoke main test test-verbose \
        ...
        benchmark-cbramod benchmark-simpleconv benchmark-models benchmark-streaming benchmark-dataloader compare-models task-c \
        sample-preprocess sample-inspect sample-harmonize sample-harmonize-edf \
        sample-compare-backends sample-harmonize-bids explore-data render-data-notebook explore-dataloader render-dataloader-notebook \
...
help: ## Show available targets.
	...
	@printf "\nCommon variables:\n"
	@printf "  RAW_DIR=%s\n  DATASET=%s  DATA_BACKEND=%s\n" "$(RAW_DIR)" "$(SHU_DATASET)" "$(DATA_BACKEND)"
	@printf "  HARMONIZED_SHU_MANIFEST=%s\n  SHU_TARGET_RATE=%s  HARMONIZE_WORKERS=%s\n" "$(HARMONIZED_SHU_MANIFEST)" "$(SHU_TARGET_RATE)" "$(HARMONIZE_WORKERS)"
	@printf "  HBN_ROOT=%s  HBN_TARGET_RATE=%s  HBN_WINDOW_SECONDS=%s\n" "$(HBN_ROOT)" "$(HBN_TARGET_RATE)" "$(HBN_WINDOW_SECONDS)"
	@printf "  SHU_TARGET_JOB_GIB=%s  HBN_TARGET_JOB_GIB=%s  MAX_RECORDINGS_PER_JOB=%s\n" "$(SHU_TARGET_JOB_GIB)" "$(HBN_TARGET_JOB_GIB)" "$(HARMONIZE_MAX_RECORDINGS_PER_JOB)"
	...
	@printf "  DATALOADER_BACKEND=%s  DATALOADER_NUM_WORKERS=%s\n" "$(DATALOADER_BACKEND)" "$(DATALOADER_NUM_WORKERS)"
	@printf "  OVERWRITE=0|1  RESUME=0|1  HARMONIZE_PROGRESS=0|1\n"
	@printf "  SKIP_INVALID=0|1  SHU_EDF_SKIP_INVALID=0|1  HBN_SKIP_INVALID=0|1\n"
	@printf "  STRICT=0|1  PYTEST_ARGS='...'\n\n"


########################################################################
##### Installation, syncing, and dependency management
########################################################################
...
preprocess: ## Preprocess the full SHU-MI MATLAB archive into HDF5.
	$(UV) run $(PYTHON) -m main preprocess --raw-dir "$(RAW_DIR)" --output "$(SHU_DATASET)" $(PREPROCESS_FLAGS)

inspect-data: ## Validate shapes, classes, subject splits, leakage, and paper sample count.
	$(UV) run $(PYTHON) -m main inspect-data --data "$(SHU_DATASET)" $(INSPECT_STRICT_FLAG)

harmonize-shu: ## Parallel SHU-MI MAT harmonization with a rank-0 tqdm progress bar.
	$(HARMONIZE_ENV) $(UV) run $(PYTHON) -m main harmonize-shu --source mat --raw-dir "$(RAW_DIR)" --output-dir "$(HARMONIZED_SHU_DIR)" --target-sampling-rate $(SHU_TARGET_RATE) --num-workers $(HARMONIZE_WORKERS) --target-job-gib $(SHU_TARGET_JOB_GIB) --max-recordings-per-job $(HARMONIZE_MAX_RECORDINGS_PER_JOB) --records-per-batch $(ARROW_RECORDS_PER_BATCH) --batches-per-shard $(ARROW_BATCHES_PER_SHARD) $(SKIP_INVALID_FLAG) $(HARMONIZE_PROGRESS_FLAG) $(HARMONIZE_RESUME_FLAG) $(PREPROCESS_FLAGS)

harmonize-shu-edf: ## Parallel EDF/event harmonization; invalid recordings are skipped by default.
	$(HARMONIZE_ENV) $(UV) run $(PYTHON) -m main harmonize-shu --source edf --raw-dir "$(SHU_EDF_DIR)" --events-root "$(SHU_EVENTS_DIR)" --output-dir "$(HARMONIZED_SHU_EDF_DIR)" --target-sampling-rate $(SHU_TARGET_RATE) --num-workers $(HARMONIZE_WORKERS) --target-job-gib $(SHU_TARGET_JOB_GIB) --max-recordings-per-job $(HARMONIZE_MAX_RECORDINGS_PER_JOB) --records-per-batch $(ARROW_RECORDS_PER_BATCH) --batches-per-shard $(ARROW_BATCHES_PER_SHARD) $(SHU_EDF_INVALID_FLAG) $(HARMONIZE_PROGRESS_FLAG) $(HARMONIZE_RESUME_FLAG) $(PREPROCESS_FLAGS)

harmonize-hbn: ## Parallel HBN/BIDS harmonization through the shared recording engine.
	$(HARMONIZE_ENV) $(UV) run $(PYTHON) -m main harmonize-bids --root "$(HBN_ROOT)" --output-dir "$(HARMONIZED_HBN_DIR)" --dataset-id "$(HBN_DATASET_ID)" $(HBN_TARGET_RATE_FLAG) --window-seconds $(HBN_WINDOW_SECONDS) --stride-seconds $(HBN_STRIDE_SECONDS) $(HBN_LIMIT_FLAG) --num-workers $(HARMONIZE_WORKERS) --target-job-gib $(HBN_TARGET_JOB_GIB) --max-recordings-per-job $(HARMONIZE_MAX_RECORDINGS_PER_JOB) --records-per-batch $(ARROW_RECORDS_PER_BATCH) --batches-per-shard $(ARROW_BATCHES_PER_SHARD) $(HBN_INVALID_FLAG) $(HARMONIZE_PROGRESS_FLAG) $(HARMONIZE_RESUME_FLAG) $(PREPROCESS_FLAGS)
...
compare-backends: ## Verify HDF5 and Arrow contain numerically identical SHU samples.
	$(UV) run $(PYTHON) -m main compare-backends --hdf5 "$(SHU_DATASET)" --manifest "$(HARMONIZED_SHU_MANIFEST)"
...
sample-preprocess: ## Preprocess the bundled subject-1 sample for pipeline validation.
	$(UV) run $(PYTHON) -m main preprocess --raw-dir $(RAW_DIR) --output $(OUTPUT_DIR)/data/preprocessed/shu_mi_sample.h5 --overwrite

sample-inspect: ## Inspect the bundled incomplete sample (warnings are expected).
	$(UV) run $(PYTHON) -m main inspect-data --data $(OUTPUT_DIR)/data/processed/shu_mi_sample.h5

sample-harmonize: ## Build Arrow shards from the bundled SHU-MI MAT sample.
	$(UV) run $(PYTHON) -m main harmonize-shu --source mat --raw-dir $(RAW_DIR) --output-dir $(OUTPUT_DIR)/data/harmonized/shu_mi_sample --target-sampling-rate 200 --num-workers 1 --no-progress --records-per-batch 32 --batches-per-shard 2 --overwrite

sample-harmonize-edf: ## Reconstruct the bundled MAT sample through EDF + events.
	$(UV) run $(PYTHON) -m main harmonize-shu --source edf --raw-dir resources/data/shu-mi/edf_files --events-root resources/data/shu-mi/events --output-dir $(OUTPUT_DIR)/data/harmonized/shu_mi_edf_sample --target-sampling-rate 200 --records-per-batch 32 --batches-per-shard 2 --skip-invalid-recordings --overwrite

sample-compare-backends: sample-preprocess sample-harmonize ## Verify exact HDF5/Arrow parity on the bundled sample.
	$(UV) run $(PYTHON) -m main compare-backends --hdf5 resources/data/processed/shu_mi_sample.h5 --manifest $(OUTPUT_DIR)/data/harmonized/shu_mi_sample/manifest.parquet

sample-harmonize-bids: ## Exercise the generic BIDS reader on the bundled EDF recording.
	$(UV) run $(PYTHON) -m main harmonize-bids --root resources/data/shu-mi/edf_files --output-dir $(OUTPUT_DIR)/data/harmonized/bids_sample --dataset-id bids-poc --window-seconds 4 --stride-seconds 4 --limit-recordings 1 --num-workers 1 --no-progress --records-per-batch 32 --batches-per-shard 2 --overwrite
...
explore-dataloader: ## Open the harmonized dataloader benchmark notebook.
	$(UV) run jupyter lab "$(DATALOADER_NOTEBOOK)"

render-dataloader-notebook: ## Execute the dataloader benchmark notebook and export HTML.
	$(UV) run jupyter nbconvert --to html --execute "$(DATALOADER_NOTEBOOK)" --ExecutePreprocessor.timeout=1800 --output-dir notebooks


########################################################################
##### Models training, reproduction, and benchmarking
########################################################################

train-cbramod: ## Run one CBraMod seed with the paper-aligned configuration.
	$(UV) run $(PYTHON) -m main train --config "$(CBRAMOD_CONFIG)" --data "$(SHU_DATASET)" --data-backend "$(DATA_BACKEND)" --output-dir "$(CBRAMOD_OUTPUT_DIR)/seed_3407" $(CHECKPOINT_FLAG) $(STRICT_DATA_FLAG)

train-simpleconv: ## Run one EEGSimpleConv seed on the same processed data.
	$(UV) run $(PYTHON) -m main train --config "$(SIMPLECONV_CONFIG)" --data "$(SHU_DATASET)" --data-backend "$(DATA_BACKEND)" --output-dir "$(SIMPLECONV_OUTPUT_DIR)/seed_3407" $(STRICT_DATA_FLAG)

reproduce-cbramod: ## Run five seeds and write mean/std metrics to summary.json.
	$(UV) run $(PYTHON) -m main reproduce --config "$(CBRAMOD_CONFIG)" --data "$(SHU_DATASET)" --data-backend "$(DATA_BACKEND)" --output-dir "$(CBRAMOD_OUTPUT_DIR)" --seeds $(REPRO_SEEDS) $(CHECKPOINT_FLAG)

reproduce-simpleconv: ## Run five EEGSimpleConv seeds and aggregate the metrics.
	$(UV) run $(PYTHON) -m main reproduce --config "$(SIMPLECONV_CONFIG)" --data "$(SHU_DATASET)" --data-backend "$(DATA_BACKEND)" --output-dir "$(SIMPLECONV_OUTPUT_DIR)" --seeds $(REPRO_SEEDS)
...
benchmark-dataloader: ## Measure one complete dataloader epoch over HDF5 or Arrow.
	$(UV) run $(PYTHON) -m main benchmark-dataloader --data "$(DATALOADER_DATA)" --backend "$(DATALOADER_BACKEND)" --split "$(DATALOADER_SPLIT)" --output "$(DATALOADER_BENCHMARK_OUTPUT)" --batch-size $(DATALOADER_BATCH_SIZE) --num-workers $(DATALOADER_NUM_WORKERS) --prefetch-factor $(DATALOADER_PREFETCH_FACTOR) --shuffle-buffer-size $(DATALOADER_SHUFFLE_BUFFER) --device "$(DATALOADER_DEVICE)" --epoch $(DATALOADER_EPOCH) $(DATALOADER_PROGRESS_FLAG)
...
reproduce-cbramod-debug: ## Run the reproduction command without requiring all 25 subjects.
	$(UV) run $(PYTHON) -m main reproduce --config "$(CBRAMOD_CONFIG)" --data "$(SHU_DATASET)" --data-backend "$(DATA_BACKEND)" --output-dir "$(CBRAMOD_OUTPUT_DIR)" --seeds $(REPRO_SEEDS) $(CHECKPOINT_FLAG) --allow-incomplete-data
...
clean-data: ## Remove generated HDF5/Arrow data; raw data is preserved.
	rm -rf $(OUTPUT_DIR)/data/processed $(OUTPUT_DIR)/data/harmonized $(OUTPUT_DIR)/data/exploration
