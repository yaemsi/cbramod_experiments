...
The repository includes one real SHU-MI subject/session in MAT, EDF, and event-TSV forms. It supports fast local validation without downloading the full dataset. EDF ingestion is strict by default; for corpus audits, `SKIP_INVALID=1` records malformed recordings in `source_audit.json` and continues:
